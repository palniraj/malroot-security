...
Fixes:
* Quarantine no longer bloats the database. Backups are stored base64-encoded in a LONGTEXT column, which adds about a third to the size, and nothing prevented the same file being stored again on a second cleanup. Removing the kit from one site put 58.4 MB into the table and took a 17 MB database export to 76 MB: a 16 MB reverse-shell toolkit became 21 MB of base64, stored twice. Files above 2 MB (filterable via malroot_max_backup_bytes) now have their identity recorded - size, permissions and a SHA-256 - without a copy of the contents, and identical backups are de-duplicated rather than appended. The same cleanup now uses 0.15 MB instead of 58.4 MB, and a repeat run adds nothing. Restoring a metadata-only record explains what was removed and gives you the hash to identify it in your own backup, instead of silently writing an empty file.
* Cleanup now restores the WordPress permalink rules if they are missing. Removing the cloaking rules from .htaccess is only half the job: the attacker on cityagecare.com had replaced the whole file, so the "# BEGIN WordPress" block was already gone and stripping the cloak left nothing to route requests to index.php. Every page returned a bare Apache 404 straight after a successful cleanup, which looks worse than the infection. The rules come from WordPress itself, and the environment sniff in got_mod_rewrite() is bypassed when it refuses, because apache_get_modules() does not exist under LiteSpeed or PHP-FPM and would block the repair on exactly that kind of hosting.
* The backdoor scanner now also walks wp-admin and wp-includes. It previously covered wp-content only, and on cityagecare.com a second copy of the obfuscated payload was sitting at wp-includes/SimplePie/library/SimplePie/Files.php - 3.87 MB, byte-identical to the one in the fake plugin folder, next to a genuine 529-byte File.php. Nothing looked there, which is exactly why attackers use core paths. Core and vendored libraries are checked with the unambiguous rules only (obfuscator banner, chunk-assembled payload), because the heuristic rules produce noise against legitimate code such as functions.php, class-wpdb.php, PHPMailer and the ID3 parsers, and rules describing rogue-plugin behaviour are meaningless against WordPress itself.
* Accounts that Malroot had already disabled were never deleted. Admin Guard neutralises a rogue administrator by removing its role, which stops it instantly, but the cleanup and the scanner both searched for administrators — so once the role was gone the account became invisible to them and stayed on the site with role "None" and no email, looking like an unfinished cleanup. New rule UA-007 reports these, and cleanup now removes them.
